...
import { ChangeDetectionStrategy, Component, computed, contentChild, contentChildren, effect, ElementRef, forwardRef, inject, InjectionToken, input, NgModule, numberAttribute, output, untracked, ViewEncapsulation } from '@angular/core';
...
@Component({
    ...
    template: `
        @for (panel of panels(); track panel; let i = $index) {
            <div [vxBind]="ptm('panel')" [class]="cn(cx('panel'), panelStyleClass())" [style]="panelStyle()" tabindex="-1">
...
export class Splitter extends BaseComponent<SplitterPassThrough> {
     ...
     * @group Props
     */
    gutterSize = input<number, unknown>(4, { transform: numberAttribute });
    /**
     * Step factor to increment/decrement the size of the panels while pressing the arrow keys.
     * @group Props
     */
    step = input<number, unknown>(5, { transform: numberAttribute });
    /**
     ...
     * @group Props
     */
    panelSizes = input<number[] | undefined>();
    /**
    ...
    panelChildren = contentChildren<ElementRef>('panel', { descendants: false });

    splitter = contentChild(forwardRef(() => Splitter));

    nestedState = computed(() => this.splitter());

    panels = computed<any[]>(() => [...this.templates().map((item) => item.template), ...this.panelChildren()]);
    ...
    constructor() {
        ...
        // Replaces the former panelSizes setter side effect: apply programmatic size
        // changes to the rendered panels (#17).
        effect(() => {
            const val = this.panelSizes();

            if (val) {
                untracked(() => this.applyPanelSizes(val));
            }
        });
    }

    private applyPanelSizes(val: number[]) {
        ...
        if (isPlatformBrowser(this.platformId)) {
            if (this.panels() && this.panels().length) {
                ...
                if (!initialized) {
                    ...
                    this.panels().map((panel, i) => {
                        let panelInitialSize = this._panelSizes.length - 1 >= i ? this._panelSizes[i] : null;
                        let panelSize = panelInitialSize || 100 / this.panels().length;
