...
export class SelectButton extends BaseEditableHolder<SelectButtonPassThrough> implements AfterViewChecked {
     ...
     * @group Props
     */
    unselectable = input<boolean, unknown>(false, { transform: booleanAttribute });
    /**
    ...
    /**
     * Resolved allowEmpty state: setting `unselectable` takes precedence over `allowEmpty`.
     */
    ...
    _itemTemplate = computed<TemplateRef<SelectButtonItemTemplateContext> | undefined>(() => this.templates().find((item) => item.getType() === 'item')?.template);
