...
export class ToastItem extends BaseComponent<ToastPassThrough> {
    message = input<ToastMessageOptions | null | undefined>();

    index = input<number | null | undefined, unknown>(undefined, { transform: numberAttribute });

    life = input<number | undefined, unknown>(undefined, { transform: numberAttribute });

    template = input<TemplateRef<ToastMessageTemplateContext> | undefined>();

    headlessTemplate = input<TemplateRef<ToastHeadlessTemplateContext> | undefined>();

    showTransformOptions = input<string | undefined>();

    hideTransformOptions = input<string | undefined>();

    showTransitionOptions = input<string | undefined>();

    hideTransitionOptions = input<string | undefined>();
...
export class Toast extends BaseComponent<ToastPassThrough> {
     ...
     * @group Props
     */
    breakpoints = input<{ [key: string]: any } | undefined>();
    /**
     ...
     * back to the `message` key, matching the pre-signal `ngAfterContentInit` behavior.
     */
    private _templateMap = computed(() => {
        ...
        for (const item of this.templates()) {
            switch (item.getType()) {
                case 'headless':
                    map['headless'] = item.template;
                    break;

                case 'message':
                default:
                    map['message'] = item.template;
                    break;
            }
        }
    ...
    _template = computed(() => this._templateMap()['message'] as TemplateRef<ToastMessageTemplateContext> | undefined);

    _headlessTemplate = computed(() => this._templateMap()['headless'] as TemplateRef<ToastHeadlessTemplateContext> | undefined);
