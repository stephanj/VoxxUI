import { AppDocSectionText } from '@/components/doc/app.docsectiontext';
import { ChangeDetectionStrategy, Component } from '@angular/core';
import { TagModule } from 'voxx-ui/tag';

@Component({
    changeDetection: ChangeDetectionStrategy.Eager,
    selector: 'v20-deprecations-doc',
    standalone: true,
    imports: [TagModule, AppDocSectionText],
    template: `
        <app-docsectiontext>
            <p>The following items are marked as deprecated.</p>
            <div class="doc-tablewrapper">
                <table class="doc-table">
                    <thead>
                        <tr>
                            <th>API</th>
                            <th class="whitespace-nowrap">Deprecated Since</th>
                            <th>Replacement</th>
                            <th>Removal</th>
                            <th>Status</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
                            <td>@voxxui/themes</td>
                            <td>v20</td>
                            <td>@primeuix/themes</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>vxTemplate</td>
                            <td>v20</td>
                            <td>ng-template with a template reference variable</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>styleClass *(for host enabled components)</td>
                            <td>v20</td>
                            <td>class</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>Global inputStyle config</td>
                            <td>v20</td>
                            <td>inputVariant</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>CamelCase Selectors</td>
                            <td>v20</td>
                            <td>Kebab case</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>vxButton iconPos, loadingIcon, icon and label properties</td>
                            <td>v20</td>
                            <td><i>vxButtonIcon</i> and <i>vxButtonLabel</i> directives</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>vxButton buttonProps property</td>
                            <td>v20</td>
                            <td>Use button properties directly on the element</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>p-button badgeClass property</td>
                            <td>v20</td>
                            <td><i>badgeSeverity</i> property</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>AutoComplete minLength property</td>
                            <td>v20</td>
                            <td>minQueryLength</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>OrganizationChart preserveSpace property</td>
                            <td>v20</td>
                            <td>Obselete property, had no use</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>Paginator dropdownAppendTo property</td>
                            <td>v20</td>
                            <td>appendTo</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>Message text and escape properties</td>
                            <td>v20</td>
                            <td>Content projection</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>Password maxLength property</td>
                            <td>v20</td>
                            <td><i>maxlength</i> property</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>TreeSelect containerStyle/containerStyleClass properties</td>
                            <td>v20</td>
                            <td>style and class</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>Table responsiveLayout property</td>
                            <td>v20</td>
                            <td>Always defaults to scroll, stack mode needs custom implementation</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>TreeSelect default template</td>
                            <td>v20</td>
                            <td><i>value</i> template</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>vxBadge directive</td>
                            <td>v20</td>
                            <td><i>OverlayBadge</i> component</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>clearFilterIcon template of Table</td>
                            <td>v20</td>
                            <td>Obsolete, not utilized.</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                        <tr>
                            <td>Inplace closable property.</td>
                            <td>v20</td>
                            <td>Use templating with <i>closeCallback</i>.</td>
                            <td>v22</td>
                            <td><vx-tag value="deprecated" severity="warn" /></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </app-docsectiontext>
    `
})
export class DeprecationsDoc {}
